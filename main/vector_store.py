...
class QdrantVectorStore:
    ...
    def add_documents(self, documents: List[Dict[str, Any]], embed_batch_size: int = 64):
        """Add documents to vector store với batch encoding để tăng tốc trên CPU"""
        try:
            # Lọc document hợp lệ trước
            valid_docs = [doc for doc in documents if doc.get("content", "").strip()]
            print(f"Embedding {len(valid_docs)} documents (batch_size={embed_batch_size})...")

            # Batch encode toàn bộ corpus cùng lúc — nhanh hơn 3-5x so với từng cái một
            all_contents = [doc["content"] for doc in valid_docs]
            ...
            # Tạo points từ embeddings đã có
            points = []
            for doc, embedding in zip(valid_docs, all_embeddings):
                point = PointStruct(
                    id=str(uuid.uuid4()),
