...
class RAGEvaluator:
    ...
    def calculate_average_precision(self, retrieved_docs: List[str], relevant_docs: List[str]) -> float:
        ...
        matched_relevant = set()  # track để không đếm trùng
        precision_sum = 0.0
        
        for i, doc in enumerate(retrieved_docs):
            for rel_doc in relevant_docs:
                if rel_doc in doc and rel_doc not in matched_relevant:
                    matched_relevant.add(rel_doc)
                    precision_sum += len(matched_relevant) / (i + 1)
                    break
